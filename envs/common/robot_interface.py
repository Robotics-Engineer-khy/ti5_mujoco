...
import mujoco
#电机motor/actuator，关节joint；qpos/qvel对应的是关节,ctrl对应的是电机，actuator_force = ctrl_signal * gear_ratio
class RobotInterface(object):
    ...
    def get_rfoot_floor_contacts(self):
        ...
        for i,c in enumerate(contacts):
            #print(mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_GEOM, c.geom1)," with ",mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_GEOM, c.geom2))
    ...
    def get_lfoot_grf(self):
        ...
        for i, con in left_contacts:
            c_array = np.zeros(6, dtype=np.float64)#力大小+扭矩大小（3+3）
            mujoco.mj_contactForce(self.model, self.data, i, c_array)
    ...
    def get_lfoot_body_vel(self, frame=0):
        ...
        return lfoot_vel#lfoot_vel依次为：线速度+角速度
    ...
    def check_self_collisions(self):
        ...
        for i,c in enumerate(contacts):
            ...
            '''
                ...
                body_id_1 = self.model.geom_bodyid[c.geom1]
                body_id_2 = self.model.geom_bodyid[c.geom2]
                body_name_1 = mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_BODY, body_id_1)
                body_name_2 = mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_BODY, body_id_2)
                # 打印碰撞体的名称
                print(f"Collision between: {body_name_1} and {body_name_2}")
            '''
