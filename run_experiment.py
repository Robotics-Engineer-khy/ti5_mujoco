...
def run_experiment(args):
    ...
    if args.continued:
        ...
        critic = FF_V(obs_dim)#估算当前状态的价值（即reward）
        #获取初始策略网络下的num_procs条轨迹数据集（每条轨迹长input_norm_steps/num_procs）（注意此步并未用到critic）
        with torch.no_grad():
            #只收集state轨迹集，第一次训练网络时的初始均值和标准差（由大于max_traj_len的轨迹数据统计得）
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_norm_steps", type=int, default=100000)#计算初始policy/critic均值方差时的总轨迹长度
    ...
    parser.add_argument("--minibatch_size", type=int, default=128, help="Batch size for PPO updates")
    ...
    parser.add_argument("--num_procs", type=int, default=8, help="Number of threads to train on")
    ...
    parser.add_argument("--eval_freq", required=False, default=50, type=int, help="Frequency of performing evaluation")
    parser.add_argument("--continued", required=False, default="./logs_dir/", type=str, help="path to pretrained weights")#logs_dir
